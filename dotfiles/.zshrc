...
function parametriza_zsh_comun() {
  ...
  # la transición lsd → eza sea visualmente continua. Sintaxis tipo
  # LS_COLORS (38;5;N para 256-color). Override de LS_COLORS donde aplica.
  ...
  # Usar eza como reemplazo moderno de ls (con icons + git column).
  # Si eza no está disponible, fallback a lsd (durante una temporada
  # mantenemos lsd instalado por si acaso). Sin --group-directories-first:
  # se respeta el sort alfabético interleaved (preferencia del usuario,
  # mismo orden que `lsd` directo con dir-grouping=none en su config).
...
# =============================================================================
  ...
  case "$OSTYPE" in
    ...
    (darwin|freebsd)*)
      ...
      # eza/lsd: ya configurado en parametriza_zsh_comun (con fallback a lsd).
