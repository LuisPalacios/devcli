...
| § | Purpose | Key content |
| --- | --- | --- |
| §0 | Personalización | `CUSTOMIZE` table at the very top with user-tweakable knobs. Currently exposes `CLAUDE_EXTRA_ARGS` (table of strings, default `{ '--allow-dangerously-skip-permissions' }`) — extra args spliced into every claude pane launched by AI Mode (§4), inserted **before** `--model X`. Add new knobs here as fields of `CUSTOMIZE`. Also exposes `MAC_OPTION_AS_META` (`'right'` default | `'left'` | `'both'` | `'none'`) — which macOS Option key acts as Meta and which composes characters; consumed by §3. The bootstrap end-of-install message (in `ux_summary` / `Show-UxSummary`) points dev/full users to this section. |
...
| §3 | Appearance & defaults | `initial_cols/rows`, per-OS `font_size` (mac 13 / linux 12 / windows 11), `color_scheme = 'LP-GitBash'`, plus the hardcoded `LP-*` scheme table + `pane_split_color` + `inactive_pane_hsb` (moved here from §5 so they exist on every OS — defining them only in the Windows branch left macOS/Linux logging `color_scheme="LP-GitBash" ... not found`), `scrollback_lines = 100000`, `audible_bell = 'Disabled'`, `enable_scroll_bar = true` (thumb on right edge, scales with scrollback — drag to navigate fast), `window_close_confirmation = 'NeverPrompt'`. macOS-only block setting `send_composed_key_when_{left,right}_alt_is_pressed` from `CUSTOMIZE.MAC_OPTION_AS_META` — WezTerm's defaults (left Option = Meta) make `@` (⌥+2 on Spanish ISO) untypable; iTerm2 composes on both Options, hence the difference users report. Also hosts `show_theme_picker()` (the `CTRL+SHIFT+E` fuzzy theme picker that merges user `LP-*` schemes with bundled). |
